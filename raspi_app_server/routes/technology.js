...
techRoutes.post('/', auth_1.tokenVerification, (req, res) => {
    const body = req.body;
    technology_1.Technology.create(body).then(techDB => {
        res.json({
            ok: true,
            technology: techDB
        });
...
techRoutes.post('/update/:id', (req, res) => {
    ...
    technology_1.Technology.findByIdAndUpdate(id, tech, { new: true }, (err, techDB) => {
        if (err)
            throw err;
        if (!techDB) {
            res.json({
...
techRoutes.get('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const tech = yield technology_1.Technology.find().exec();
    res.json({
        ok: true,
        tech
    });
